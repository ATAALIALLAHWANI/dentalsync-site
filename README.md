...
**No phone number and no Facebook page URL were found anywhere in the
`D:\DentalSync` project files or in this project's chat history** — the
Facebook content strategy document only uses "DentalSync" as a placeholder
page name. So both are left blank on purpose. While they're blank, the
matching contact button on the site is automatically hidden (the "Call or
message us" / "Message us on Facebook" buttons), and a small note appears
in their place explaining what to add and where. Nothing was invented.
...
1. Push this project to a new GitHub repository:
   ...
   git init
   git add .
   git commit -m "DentalSync marketing website"
   git branch -M main
...
2. Go to [vercel.com](https://vercel.com) and sign in (free Hobby plan is
   enough).
3. Click **Add New → Project**, then import the GitHub repository you just
   pushed.
...
5. Click **Deploy**. In under a minute you'll get a live URL like
   `https://dentalsync.vercel.app` (or `https://your-repo-name.vercel.app`
   if that exact name is taken — you can rename the project in Vercel's
   settings to get a nicer subdomain).
...
- [ ] Add your real phone number and Facebook URL to `src/config.js`
- [ ] Update `SITE.url` in `src/config.js` and the two URLs in
      `sitemap.xml` / `robots.txt` once you know your final domain
      (or keep `dentalsync.vercel.app` if that's what you'll use)
